...
Partial Class reports_URL_sent
    ...
    Protected Sub BindGridview()
        ...
        Try
            ...
            Dim dstReportUrlSent As DataSet = (New clsDLBookingReport()).GetReportUrlSent(intCompanyId, strSearchByFromDate, strSearchByToDate)
            GRID_URL_sent.DataSource = dstReportUrlSent
            GRID_URL_sent.DataBind()
    ...
    ''' <summary>
    ''' PageIndexChanging event of the GRID_URL_sent
    ''' </summary>
    Protected Sub GRID_URL_sent_PageIndexChanging(ByVal sender As Object, ByVal e As GridViewPageEventArgs)

        Try
            GRID_URL_sent.PageIndex = e.NewPageIndex
            BindGridview()
        Catch ex As Exception
            HttpContext.Current.Trace.Warn("Exception message:  ::", ex.Message)
            HttpContext.Current.Trace.Warn("Error Stack Trace ::", ex.StackTrace)
    ...
    ''' <summary>
    ''' SelectedIndexChanged event of the GRID_URL_sent
    ''' </summary>
    Protected Sub GRID_URL_sent_SelectedIndexChanged(ByVal sender As Object, ByVal e As GridViewSelectEventArgs)
        Try
            'Trace.Warn("Value = " + objFunction.ReturnString(GRID_URL_sent.DataKeys(e.NewSelectedIndex).Value))
            Session("BookingId") = objFunction.ReturnString(GRID_URL_sent.DataKeys(e.NewSelectedIndex).Value)
